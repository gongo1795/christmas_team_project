document.addEventListener('DOMContentLoaded', () => {
    ...
    const ornamentsCollection = () => (window.db ? window.db.collection('tree-ornaments') : null);

    function ensureDb() {
        if (!window.db) {
            alert('Firebase 설정이 필요합니다. tree-decorator.html의 firebaseConfig를 채워주세요.');
            return false;
        }
        return true;
    }
    ...
    inputForm.addEventListener('submit', async (e) => {
        ...
        await addOrnamentToTree(memoDropPosition.type, memoDropPosition.left, memoDropPosition.top, memo);

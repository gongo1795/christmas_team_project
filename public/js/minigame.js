...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadSnowClickerGame(gameArea) {
        ...
        gameArea.innerHTML = `
            <div id="clicker-controls" style="width: 100%; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="text-align: right;">
                    ...
                    <div id="clickerBestScoreDisplay" style="color: gold; font-size: 0.9em;">최고 점수(내 컴퓨터): 0</div>
        ...
        rankingSection.innerHTML = `
            ...
            <p class="ranking-desc">이 Firebase 프로젝트 전체에서 등록된 상위 10명의 점수입니다.</p>
            <div class="ranking-controls">
                <button id="refreshClickerRanking" class="button-green">랭킹 새로고침</button>
            </div>
            <ol id="clickerRankingList" class="ranking-list">
                <li>아직 등록된 점수가 없어요.</li>
            </ol>
        `;
        gameArea.appendChild(rankingSection);
        ...
        async function loadClickerRanking() {
            ...
            try {
                const snapshot = await window.db
                    .collection('snow-clicker-scores')
                    .orderBy('score', 'desc')
                    .limit(10)
                    .get();
        ...
        async function saveClickerScoreToRanking(scoreToSave) {
            if (scoreToSave <= 0) return;
            if (!hasDb(rankingListEl)) return;
            ...
            nickname = nickname.trim().slice(0, 10) || '익명';
            localStorage.setItem('snowClickerNickname', nickname);

            try {
                await window.db.collection('snow-clicker-scores').add({
                    nickname,
                    score: scoreToSave,
                    difficulty: currentDifficulty,
                    createdAt: window.firestoreTimestamp
                ...
                await loadClickerRanking();
            } catch (error) {
                console.error('눈송이 클릭 랭킹 저장 오류', error);
                alert('랭킹을 저장하는 중 오류가 발생했어요.');
            }
        }
        ...
        function endGame() {
            ...
            if (score > 0) {
                saveClickerScoreToRanking(score);
            }
        }
